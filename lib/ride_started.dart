...
class RideStarted extends StatefulWidget {
  final times, type,id;
  const RideStarted({super.key, this.times, this.type,this.id});
  @override
...
class _RideStartedState extends State<RideStarted> {
          ...
          child: Column(
            children: [
              Container(
                  ...
                  child: Center(child: SvgPicture.asset("assets/ride_started.svg"))),
              SizedBox(
                height: Dim().d32,
              ),
  ...
  void completeride(id, time) async {
    ...
    var result = await STM().postWithToken(ctx, Str().processing, 'proceed_to_unload', body, usertoken);
    var success = result['success'];
    if (success) {
      STM().displayToast(result['message']);
      STM().replacePage(ctx, Destination(id: id,));
    } else {
