...
class Destination extends StatefulWidget {
  final times, type,id;
  const Destination({super.key, this.times, this.type,this.id});
  @override
...
class _DestinationState extends State<Destination> {
          ...
          child: Column(
            children: [
              Container(
                  ...
                  child: Center(child: SvgPicture.asset("assets/destination.svg"))),
              SizedBox(
                height: Dim().d32,
              ),
              ...
              TweenAnimationBuilder<Duration>(
                  ...
                  tween: Tween(
                      begin: Duration(minutes: widget.type == 'view' ? diff!.inMinutes : 0, seconds: widget.type == 'view' ? diff!.inSeconds % 60 : 0),
              ...
              Center(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                      onPressed: () {
                        completeride(widget.id,'$minutes:$seconds');
                      },
  ...
  void completeride(id, time) async {
    FormData body = FormData.fromMap({
      'request_id': id,
      'time': time,
    });
    var result = await STM().postWithToken(ctx, Str().processing, 'complete_ride', body, usertoken);
    var success = result['success'];
    if (success) {
      STM().displayToast(result['message']);
      STM().successDialogWithReplace(ctx, result['message'],RideComplete(id: widget.id,detail: result['data']));
    } else {
